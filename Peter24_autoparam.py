...
light_water = 0.125 #need to change for Paul
...
if os.path.isfile("Peter Parameterization/results.csv"):
    ...
    resp_docr_col = [random.uniform(0.0001,0.005) for _ in range(n_runs-1)]
    ...
    light_doc_col = [random.uniform(.01, 0.04) for _ in range(n_runs-1)]
    light_doc_col.insert(0, light_doc)
    light_water_col = [random.uniform(.12, .225) for _ in range(n_runs-1)]
...
# Save each variable with depth as index, time as columns
    ...
    pd.DataFrame(temp, index=depth_labels, columns=times).to_csv(f"{run_folder}/temp.csv")
    pd.DataFrame(o2, index=depth_labels, columns=times).to_csv(f"{run_folder}/do.csv")
