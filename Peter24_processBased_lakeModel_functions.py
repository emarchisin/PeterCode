...
import datetime
from Peter24_ancillary_functions import calc_cc, buoyancy_freq, center_buoyancy
import random 
import torch
...
def atmospheric_module(
    ...
    o2[0] = (o2[0] +  # m/s g/m3 m2   m/s g/m3 m2 s
        (piston_velocity * (do_sat_calc(u[0], 982.2, altitude = 516) - o2[0]/volume[0]) * area[0] ) * dt)
...
def run_wq_model(
  ...
  TP_fillvals = tuple(phosphorus_data.observation.values[[0,-1]])
  TP = interp1d(phosphorus_data.dt.values, phosphorus_data.observation.values, kind = "linear", fill_value=TP_fillvals, bounds_error=False)
